...
const Mdocuments = ({ route }) => {
  ...
  const handleContinue = async () => {
    ...
    if (!document) {
      Alert.alert('Error', 'Please upload your medical documents.');
      return;
    }
    ...
    try {
      ...
      // Store only metadata and GoFile URL in Firebase
      const vetUpdate = {
        "Medical Documents": {
          id: docId,
          name: document.name,
      ...
      navigation.navigate('Cdocuments', { vetId });
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.instruction}>
        Upload your medical license or certification (PDF format, max 5MB)
      </Text>
