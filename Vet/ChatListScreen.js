...
const ChatListScreen = ({ route }) => {
  ...
  // Update notification badge whenever conversations change
  useEffect(() => {
    const hasUnseen = conversations.some(chat => chat.unseen);
    setHasUnseenMessages(hasUnseen);
  }, [conversations, setHasUnseenMessages]);
        ...
        if (chatData) {
          ...
          Object.entries(chatData).forEach(([chatId, messages]) => {
            ...
            chats.push({
              ...
              otherUserProfilePic: otherUser.profilePic || 'https://via.placeholder.com/40',
